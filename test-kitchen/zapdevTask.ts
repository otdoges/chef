...
import { firecrawlTool } from 'zapdev-agent/tools/firecrawl';
...
export async function zapdevTask(model: ZapdevModel, outputDir: string, userMessage: string): Promise<ZapdevResult> {
  ...
  const { numDeploys, usage, success } = await withConvexBackend(backendDir, async (backend) => {
    ...
    const opts: SystemPromptOptions = {
      ...
      // TODO: We need to set up a Convex deployment running the `zapdev`
...
async function invokeGenerateText(model: ZapdevModel, opts: SystemPromptOptions, context: UIMessage[]) {
  return traced(
    async (span) => {
