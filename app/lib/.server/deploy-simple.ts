...
export async function deploy({ request }: ActionFunctionArgs) {
  try {
    ...
    if (globalThis.process.env.ZAPDEV_DEPLOY_SECRET) {
      zapdevDeploySecret = globalThis.process.env.ZAPDEV_DEPLOY_SECRET;
    }
    ...
    if (zapdevDeploySecret) {
      headers['convex-zapdev-deploy-secret'] = zapdevDeploySecret;
    }
