...
export function resendProxyGuidelines(options: SystemPromptOptions) {
  ...
  <bundled_resend_proxy_guidelines>
    Apps in the Zapdev environment come with a small number of emails
    to send via the Resend API! The Zapdev environment ONLY supports
    sending emails to a user's verified email address. For example,
    if a developer signs into Zapdev with a GitHub account with email
    test@example.com, their apps built on Zapdev can only use the
    Convex Resend proxy to send emails to test@example.com. Sending
    to any other email address will result in an error.

    Emails from the Resend proxy will always come from "Zapdev Notifications
    <{DEPLOYMENT_NAME}@convexzapdev.app>". The Resend SDK still requires
