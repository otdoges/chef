...
const downloadCommand = new Command('download')
  ...
  .action(async (chatUuid: string, options: DownloadOptions) => {
    ...
    const zapdevAdminToken = process.env.CHEF_EVAL_TOKEN;
    if (!zapdevAdminToken) {
      log('Error: Missing required environment variables');
      log('Please set CHEF_EVAL_TOKEN in your .env.local file');
      process.exit(1);
    }
    ...
    const response = await fetch(`${zapdevSiteUrl}/__debug/download_messages`, {
      ...
      headers: {
        'X-Chef-Eval-Token': zapdevAdminToken,
      },
    });
    if (!response.ok) {
...
program.name('zapdevshot').description('Zapdev AI CLI').addCommand(generateCommand).addCommand(downloadCommand);
