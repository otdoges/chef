...
npx zapdevshot generate "Let's make a chat app" --messages-file foo.json --prod | jq
```

Just save the messages you get from a prompt in prod

```
npx zapdevshot "Let's make a chat app!" --messages-file foo.json --prod
```
...
# and then in another terminal
npx zapdevshot generate "Let's make a chat app" --dev --no-headless
```
