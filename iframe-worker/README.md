The vite server of the template/snapshot vite app (the initial state of every Zapdev app)
injects code during development only so a Zapdev app in an iframe can listen to postMessage
messages from its parent Zapdev window.
